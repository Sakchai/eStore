...
namespace QNet.Web.Infrastructure.Installation
{
    /// <summary>
    ...
    public partial class InstallationLocalizationService : IInstallationLocalizationService
    {
        ...
        public virtual InstallationLanguage GetCurrentLanguage()
        {
            ...
            //try to get cookie
            var cookieName = $"{QNetCookieDefaults.Prefix}{QNetCookieDefaults.InstallationLanguageCookie}";
            ...
            foreach (var filePath in _fileProvider.EnumerateFiles(_fileProvider.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
            {
                ...
                foreach (XmlNode resNode in resources)
                {
                    ...
                    if (resNameAttribute == null)
                        throw new QNetException("All installation resources must have an attribute Name=\"Value\".");
                    var resourceName = resNameAttribute.Value.Trim();
                    if (string.IsNullOrEmpty(resourceName))
                        throw new QNetException("All installation resource attributes 'Name' must have a value.'");

                    if (resValueNode == null)
                        throw new QNetException("All installation resources must have an element \"Value\".");
