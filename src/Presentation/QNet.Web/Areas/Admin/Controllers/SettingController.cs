...
using QNet.Services.Media.RoxyFileman;
...
namespace QNet.Web.Areas.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
    {
        ...
        private readonly QNetConfig _config;
        ...
        public virtual IActionResult ChangeStoreScopeConfiguration(int storeid, string returnUrl = "")
        {
            ...
            if (store != null || storeid == 0)
            {
                _genericAttributeService
                    .SaveAttribute(_workContext.CurrentCustomer, QNetCustomerDefaults.AdminAreaStoreScopeConfigurationAttribute, storeid);
            }
        ...
        public virtual IActionResult ChangeEncryptionKey(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(newEncryptionPrivateKey) || newEncryptionPrivateKey.Length != 16)
                    throw new QNetException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TooShort"));

                var oldEncryptionPrivateKey = securitySettings.EncryptionKey;
                if (oldEncryptionPrivateKey == newEncryptionPrivateKey)
                    throw new QNetException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TheSame"));
        ...
        public virtual IActionResult ToggleFullText(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                if (!_fulltextService.IsFullTextSupported())
                    throw new QNetException(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.FullTextSettings.NotSupported"));
        ...
        public virtual IActionResult UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var headCodePath = _fileProvider.GetAbsolutePath(string.Format(QNetCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration), QNetCommonDefaults.HeadCodeFileName);

                if (!_fileProvider.FileExists(headCodePath))
                {
                    throw new Exception(string.Format(_localizationService.GetResource("Admin.Configuration.Settings.GeneralCommon.FaviconAndAppIcons.MissingFile"), QNetCommonDefaults.HeadCodeFileName));
                }
                ...
                var oldFaviconIconPath = _fileProvider.GetAbsolutePath(string.Format(QNetCommonDefaults.OldFaviconIconName, _storeContext.ActiveStoreScopeConfiguration));
