...
namespace QNet.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual string ValidateCustomerRoles(IList<CustomerRole> customerRoles, IList<CustomerRole> existingCustomerRoles)
        {
            ...
            if (rolesToAdd.Where(role => role.SystemName != QNetCustomerDefaults.RegisteredRoleName).Any() || rolesToDelete.Any())
            {
            ...
            var isInGuestsRole = customerRoles.FirstOrDefault(cr => cr.SystemName == QNetCustomerDefaults.GuestsRoleName) != null;
            var isInRegisteredRole = customerRoles.FirstOrDefault(cr => cr.SystemName == QNetCustomerDefaults.RegisteredRoleName) != null;
        ...
        {
            var customers = _customerService.GetAllCustomers(customerRoleIds: new[] { _customerService.GetCustomerRoleBySystemName(QNetCustomerDefaults.AdministratorsRoleName).Id });
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (newCustomerRoles.Any() && newCustomerRoles.FirstOrDefault(c => c.SystemName == QNetCustomerDefaults.RegisteredRoleName) != null &&
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_customerSettings.DateOfBirthEnabled)
                    _genericAttributeService.SaveAttribute(customer, QNetCustomerDefaults.DateOfBirthAttribute, model.DateOfBirth);
                ...
                foreach (var customerRole in newCustomerRoles)
                {
                    ...
                    if (customerRole.SystemName == QNetCustomerDefaults.AdministratorsRoleName && !_workContext.CurrentCustomer.IsAdmin())
                        continue;
                ...
                if (customer.IsVendor() && customer.VendorId == 0)
                {
                    var vendorRole = customer
                        .CustomerRoles
                        .FirstOrDefault(x => x.SystemName == QNetCustomerDefaults.VendorsRoleName);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                        ...
                        if (!string.IsNullOrEmpty(model.VatNumber))
                        {
                            if (!model.VatNumber.Equals(prevVatNumber, StringComparison.InvariantCultureIgnoreCase))
                            {
                                _genericAttributeService.SaveAttribute(customer,
                                    QNetCustomerDefaults.VatNumberStatusIdAttribute,
                    ...
                    foreach (var customerRole in allCustomerRoles)
                    {
                        ...
                        if (customerRole.SystemName == QNetCustomerDefaults.AdministratorsRoleName &&
                        ...
                        else
                        {
                            ...
                            if (customerRole.SystemName == QNetCustomerDefaults.AdministratorsRoleName && !SecondAdminAccountExists(customer))
                            {
        ...
        public virtual IActionResult Impersonate(int id)
        {
            ...
            _customerService.UpdateCustomer(customer);
            _genericAttributeService.SaveAttribute<int?>(_workContext.CurrentCustomer, QNetCustomerDefaults.ImpersonatedCustomerIdAttribute, customer.Id);
        ...
        public virtual IActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(customer.Email))
                    throw new QNetException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new QNetException("Customer email is not valid");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new QNetException("Email subject is empty");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new QNetException("Email body is empty");

                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                    throw new QNetException("Email account can't be loaded");
        ...
        public virtual IActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                if (!_forumSettings.AllowPrivateMessages)
                    throw new QNetException("Private messages are disabled");
                if (customer.IsGuest())
                    throw new QNetException("Customer should be registered");
                if (string.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new QNetException("PM subject is empty");
                if (string.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new QNetException("PM message is empty");
        ...
        public virtual IActionResult LoadCustomerStatistics(string period)
        {
            ...
            var searchCustomerRoleIds = new[] { _customerService.GetCustomerRoleBySystemName(QNetCustomerDefaults.RegisteredRoleName).Id };
