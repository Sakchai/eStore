...
namespace QNet.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        protected virtual void ParseAndSaveCheckoutAttributes(IList<ShoppingCartItem> cart, IFormCollection form)
        {
            ...
            _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, QNetCustomerDefaults.CheckoutAttributes, attributesXml, _storeContext.CurrentStore.Id);
        }
        ...
        protected virtual string GetProductAttributesXml(Product product, IFormCollection form, List<string> errors)
        {
            ...
            {
                var controlId = $"{QNetAttributePrefixDefaults.Product}{attribute.Id}";
                switch (attribute.AttributeControlType)
                {
                    case AttributeControlType.DropdownList:
                    case AttributeControlType.RadioList:
                    ...
                    case AttributeControlType.ImageSquares:
                        {
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                ...
                                if (selectedAttributeId > 0)
                                {
                                    ...
                                    var quantity = 1;
                                    var quantityStr = form[$"{QNetAttributePrefixDefaults.Product}{attribute.Id}_{selectedAttributeId}_qty"];
                            ...
                            if (!StringValues.IsNullOrEmpty(ctrlAttributes))
                            {
                                    ...
                                    .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                                {
                                    ...
                                    if (selectedAttributeId > 0)
                                    {
                                        //get quantity entered by customer
                                        var quantity = 1;
                                        var quantityStr = form[$"{QNetAttributePrefixDefaults.Product}{attribute.Id}_{item}_qty"];
                                        if (!StringValues.IsNullOrEmpty(quantityStr) &&
                                            (!int.TryParse(quantityStr, out quantity) || quantity < 1))
                                            errors.Add(_localizationService.GetResource("ShoppingCart.QuantityShouldPositive"));
            ...
            bool loadPicture, IFormCollection form)
        {
            ...
            if (loadPicture)
            {
                ...
                if (pictureId > 0)
                {
                    var productAttributePictureCacheKey = string.Format(QNetModelCacheDefaults.ProductAttributePictureModelKey,
        ...
        {
            var returnUrl = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, QNetCustomerDefaults.LastContinueShoppingPageAttribute, _storeContext.CurrentStore.Id);
        ...
        public virtual IActionResult Wishlist(Guid? customerGuid)
        {
            ...
            var customer = customerGuid.HasValue ?
                _customerService.GetCustomerByGuid(customerGuid.Value.ToString())
                : _workContext.CurrentCustomer;
            if (customer == null)
                return RedirectToRoute("Homepage");
