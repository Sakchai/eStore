...
CREATE TABLE `productreview_reviewtype_mapping` (
  ...
  PRIMARY KEY (`Id`),
  KEY `IX_ProductReview_ReviewType_Mapping_ProductReviewId` (`ProductReviewId`),
  KEY `IX_ProductReview_ReviewType_Mapping_ReviewTypeId` (`ReviewTypeId`),
  CONSTRAINT `FK_ProductReview_ReviewType_Mapping_ProductReview_ProductRevie3` FOREIGN KEY (`ProductReviewId`) REFERENCES `productreview` (`Id`) ON DELETE CASCADE,
  CONSTRAINT `FK_ProductReview_ReviewType_Mapping_ReviewType_ReviewTypeId` FOREIGN KEY (`ReviewTypeId`) REFERENCES `reviewtype` (`Id`) ON DELETE CASCADE
