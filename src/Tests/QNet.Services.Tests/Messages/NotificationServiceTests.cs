...
namespace QNet.Services.Tests.Messages
{
    [TestFixture]
    class NotificationServiceTests : ServiceTest
    {
        ...
        private IList<NotifyData> DeserializedDataDictionary
        {
            get
            {
                return JsonConvert.DeserializeObject<IList<NotifyData>>(_dataDictionary[QNetMessageDefaults.NotificationListKey].ToString());
            }
        }
