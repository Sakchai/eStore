...
namespace QNet.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
            ...
            ResizeType resizeType = ResizeType.LongestSide, bool ensureSizePositive = true)
        {
            ...
            //we invoke Math.Round to ensure that no white background is rendered - https://www.qnet.net/boards/t/40616/image-resizing-bug.aspx
            return new Size((int)Math.Round(width), (int)Math.Round(height));
        }
        ...
        protected virtual void DeletePictureThumbs(Picture picture)
        {
            ...
            var currentFiles = _fileProvider.GetFiles(_fileProvider.GetAbsolutePath(QNetMediaDefaults.ImageThumbsPath), filter, false);
        ...
        {
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath(QNetMediaDefaults.ImageThumbsPath);

            if (_mediaSettings.MultipleThumbDirectories)
            {
                //get the first two letters of the file name
                var fileNameWithoutExtension = _fileProvider.GetFileNameWithoutExtension(thumbFileName);
                if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > QNetMediaDefaults.MultipleThumbDirectoriesLength)
                {
                    var subDirectoryName = fileNameWithoutExtension.Substring(0, QNetMediaDefaults.MultipleThumbDirectoriesLength);
                    thumbsDirectoryPath = _fileProvider.GetAbsolutePath(QNetMediaDefaults.ImageThumbsPath, subDirectoryName);
            ...
            string storeLocation = null)
        {
            ...
            switch (defaultPictureType)
            {
                case PictureType.Avatar:
                    defaultImageFileName = _settingService.GetSettingByKey("Media.Customer.DefaultAvatarImageName", QNetMediaDefaults.DefaultAvatarFileName);
                    break;
                case PictureType.Entity:
                default:
                    defaultImageFileName = _settingService.GetSettingByKey("Media.DefaultImageName", QNetMediaDefaults.DefaultImageFileName);
                    break;
            }
