    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new QNetException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > QNetCustomerServiceDefaults.CustomerUsernameLength)
                throw new QNetException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new QNetException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
