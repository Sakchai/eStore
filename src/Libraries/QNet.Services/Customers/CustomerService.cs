...
using QNet.Core.Data.Extensions;
    ...
    public partial class CustomerService : ICustomerService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(firstName))
            {
                query = query
                    ...
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.FirstNameAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(lastName))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.LastNameAttribute &&
            ...
            if (dayOfBirth > 0 && monthOfBirth > 0)
            {
                ...
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.DateOfBirthAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(company))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.CompanyAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(phone))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.PhoneAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(zipPostalCode))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == QNetCustomerDefaults.ZipPostalCodeAttribute &&
        ...
        public virtual void DeleteCustomer(Customer customer)
        {
            ...
            if (customer.IsSystemAccount)
                throw new QNetException($"System customer account ({customer.SystemName}) could not be deleted");
        ...
        /// <returns>A customer</returns>
        public virtual Customer GetCustomerByGuid(string customerGuid)
        {
        ...
        public virtual Customer InsertGuestCustomer()
        {
            ...
            //add to 'Guests' role
            var guestRole = GetCustomerRoleBySystemName(QNetCustomerDefaults.GuestsRoleName);
            if (guestRole == null)
                throw new QNetException("'Guests' role could not be loaded");
            ...
            bool clearPaymentMethod = true)
        {
            ...
            if (clearCouponCodes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, QNetCustomerDefaults.DiscountCouponCodeAttribute, null);
                _genericAttributeService.SaveAttribute<string>(customer, QNetCustomerDefaults.GiftCardCouponCodesAttribute, null);
            }

            //clear checkout attributes
            if (clearCheckoutAttributes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, QNetCustomerDefaults.CheckoutAttributes, null, storeId);
            }

            //clear reward points flag
            if (clearRewardPoints)
            {
                _genericAttributeService.SaveAttribute(customer, QNetCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, false, storeId);
            }

            //clear selected shipping method
            if (clearShippingMethod)
            {
                _genericAttributeService.SaveAttribute<ShippingOption>(customer, QNetCustomerDefaults.SelectedShippingOptionAttribute, null, storeId);
                _genericAttributeService.SaveAttribute<ShippingOption>(customer, QNetCustomerDefaults.OfferedShippingOptionsAttribute, null, storeId);
                _genericAttributeService.SaveAttribute<PickupPoint>(customer, QNetCustomerDefaults.SelectedPickupPointAttribute, null, storeId);
            }

            //clear selected payment method
            if (clearPaymentMethod)
            {
                _genericAttributeService.SaveAttribute<string>(customer, QNetCustomerDefaults.SelectedPaymentMethodAttribute, null, storeId);
            }
        ...
        public virtual string GetCustomerFullName(Customer customer)
        {
            ...
            var firstName = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.FirstNameAttribute);
            var lastName = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.LastNameAttribute);
        ...
        public virtual string FormatUsername(Customer customer, bool stripTooLong = false, int maxLength = 0)
        {
            ...
            switch (_customerSettings.CustomerNameFormat)
            {
                ...
                case CustomerNameFormat.ShowFirstName:
                    result = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.FirstNameAttribute);
                    break;
                default:
                    break;
            }
        ...
        public virtual string[] ParseAppliedDiscountCouponCodes(Customer customer)
        {
            ...
            var existingCouponCodes = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.DiscountCouponCodeAttribute);
        ...
        public virtual void ApplyDiscountCouponCode(Customer customer, string couponCode)
        {
            ...
            //apply new value
            _genericAttributeService.SaveAttribute(customer, QNetCustomerDefaults.DiscountCouponCodeAttribute, result);
        }
        ...
        public virtual string[] ParseAppliedGiftCardCouponCodes(Customer customer)
        {
            ...
            var existingCouponCodes = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.GiftCardCouponCodesAttribute);
        ...
        public virtual void ApplyGiftCardCouponCode(Customer customer, string couponCode)
        {
            ...
            //apply new value
            _genericAttributeService.SaveAttribute(customer, QNetCustomerDefaults.GiftCardCouponCodesAttribute, result);
        }
        ...
        public virtual void DeleteCustomerRole(CustomerRole customerRole)
        {
            ...
            if (customerRole.IsSystemRole)
                throw new QNetException("System role could not be deleted");

            _customerRoleRepository.Delete(customerRole);

            _cacheManager.RemoveByPrefix(QNetCustomerServiceDefaults.CustomerRolesPrefixCacheKey);
        ...
        public virtual CustomerRole GetCustomerRoleBySystemName(string systemName)
        {
            ...
            var key = string.Format(QNetCustomerServiceDefaults.CustomerRolesBySystemNameCacheKey, systemName);
        ...
        {
            var key = string.Format(QNetCustomerServiceDefaults.CustomerRolesAllCacheKey, showHidden);
            return _cacheManager.Get(key, () =>
            {
                var query = from cr in _customerRoleRepository.Table
                            orderby cr.Name
        ...
        public virtual bool IsPasswordRecoveryTokenValid(Customer customer, string token)
        {
            ...
            var cPrt = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.PasswordRecoveryTokenAttribute);
        ...
        public virtual bool IsPasswordRecoveryLinkExpired(Customer customer)
        {
            ...
            var geneatedDate = _genericAttributeService.GetAttribute<DateTime?>(customer, QNetCustomerDefaults.PasswordRecoveryTokenDateGeneratedAttribute);
        ...
        public virtual bool PasswordIsExpired(Customer customer)
        {
            ...
            var cacheKey = string.Format(QNetCustomerServiceDefaults.CustomerPasswordLifetimeCacheKey, customer.Id);
