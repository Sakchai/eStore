    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            bool? useRewardPoints, Customer customer, decimal orderTotal)
        {
            ...
            if (!useRewardPoints.HasValue)
                useRewardPoints = _genericAttributeService.GetAttribute<bool>(customer, QNetCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, _storeContext.CurrentStore.Id);
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            if (customer != null)
            {
                var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            ...
            out decimal taxRate, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            if (customer != null)
                shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(customer, QNetCustomerDefaults.SelectedShippingOptionAttribute, _storeContext.CurrentStore.Id);
            ...
            else
            {
                ...
                if (shippingRateComputationMethods != null)
                {
                    if (!shippingRateComputationMethods.Any() && !_shippingSettings.AllowPickupInStore)
                        throw new QNetException("Shipping rate computation method could not be loaded");
