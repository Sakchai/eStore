...
namespace QNet.Services.Plugins
{
    ...
    public partial class PluginService : IPluginService
    {
        ...
        public virtual string GetPluginLogoUrl(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = QNetPluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{QNetPluginDefaults.LogoFileName}.{ext}")));
            if (string.IsNullOrWhiteSpace(logoExtension))
                return null;

            var storeLocation = _webHelper.GetStoreLocation();
            var logoUrl = $"{storeLocation}{QNetPluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{QNetPluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new QNetException(errorMessage);
                    }
                }
            }
        ...
        public virtual void InstallPlugins()
        {
            ...
            foreach (var descriptor in pluginDescriptors.OrderBy(pluginDescriptor => pluginDescriptor.DisplayOrder))
            {
                try
                {
                    ...
                    var customer = _customerService.GetCustomerByGuid(pluginToInstall.CustomerGuid);
