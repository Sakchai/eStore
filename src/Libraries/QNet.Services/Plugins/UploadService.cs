...
using QNet.Services.Themes;

namespace QNet.Services.Plugins
{
    /// <summary>
    ...
    public class UploadService : IUploadService
    {
        ...
        public UploadService(IQNetFileProvider fileProvider,
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                var uploadedItemsFileEntry = archive.Entries
                    .FirstOrDefault(entry => entry.Name.Equals(QNetPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            if (!string.IsNullOrEmpty(QNetPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(QNetPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{QNetPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{QNetPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{QNetPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    using (var unzippedEntryStream = entry.Open())
                    {
                        using (var reader = new StreamReader(unzippedEntryStream))
                        {
                            ...
                            if (isPluginDescriptor)
                            {
                                ...
                                if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(QNetVersion.CurrentVersion))
                                    throw new Exception($"This plugin doesn't support the current version - {QNetVersion.CurrentVersion}");
                            }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    //ensure that the current version of QNet is supported
                    if (!item.SupportedVersions?.Contains(QNetVersion.CurrentVersion) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{QNetPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(QNetPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{QNetPluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(QNetVersion.CurrentVersion))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(QNetPluginDefaults.UploadsTempPath);
                ...
                //you can find a sample of such descriptive file in Libraries\QNet.Core\Plugins\Samples\
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(QNetCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration));
        ...
        public class UploadedItem
        {
            ...
            /// <summary>
            /// Gets or sets supported versions of QNet
            /// </summary>
