...
using QNet.Plugin.Widgets.GoogleAnalytics.Api;
...
namespace QNet.Plugin.Widgets.GoogleAnalytics
{
    public class EventConsumer : IConsumer<OrderCancelledEvent>, IConsumer<OrderPaidEvent>, IConsumer<EntityDeletedEvent<Order>>
    {
        ...
        private void ProcessOrderEvent(Order order, bool add)
        {
            try
            {
                ...
                request.SendRequest(trans, _httpClientFactory.CreateClient(QNetHttpDefaults.DefaultHttpClient));
            }
            catch (Exception ex)
            {
