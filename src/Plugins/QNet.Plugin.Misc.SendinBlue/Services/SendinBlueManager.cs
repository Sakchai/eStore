...
using QNet.Plugin.Misc.SendinBlue.Domain;
    ...
    public class SendinBlueManager
    {
                ...
                foreach (var storeId in storeIds)
                {
                    ...
                    var csv = subscriptions.Aggregate(title, (all, subscription) =>
                    {
                        ...
                        if (customer != null)
                        {
                            ...
                            phone = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.PhoneAttribute);
                            var countryId = _genericAttributeService.GetAttribute<int>(customer, QNetCustomerDefaults.CountryIdAttribute);
                            ...
                            gender = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.GenderAttribute);
                            dateOfBirth = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.DateOfBirthAttribute);
                            ...
                            zipCode = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.ZipPostalCodeAttribute);
                            ...
                            state = _stateProvinceService.GetStateProvinceById(_genericAttributeService.GetAttribute<int>(customer, QNetCustomerDefaults.StateProvinceIdAttribute))?.Name;
                            fax = _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.FaxAttribute);
                        }
                        return $"{all}\n" +
                            $"{subscription.Email};" +
                            $"{firstName};" +
        ...
        public int? GetTemplateId(int? templateId, MessageTemplate message, EmailAccount emailAccount)
        {
            try
            {
                ...
                //or create new one
                if (emailAccount == null)
                    throw new QNetException("Email account not configured");
        ...
        public QueuedEmail GetQueuedEmailFromTemplate(int templateId)
        {
            try
            {
                ...
                if (templateId == 0)
                    throw new QNetException("Message template is empty");
        ...
        public void SendSMS(string to, string from, string text)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(to) || string.IsNullOrEmpty(text))
                    throw new QNetException("Phone number or SMS text is empty");
        ...
        public string SendSMSCampaign(int listId, string from, string text)
        {
            try
            {
                ...
                if (listId == 0 || string.IsNullOrEmpty(text) || string.IsNullOrEmpty(from))
                    throw new QNetException("List or SMS text or sender name is empty");
