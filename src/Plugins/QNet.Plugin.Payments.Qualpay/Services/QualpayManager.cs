...
using QNet.Plugin.Payments.Qualpay.Domain;
...
namespace QNet.Plugin.Payments.Qualpay.Services
{
    /// <summary>
    ...
    public class QualpayManager
    {
        ...
        private (TResult Result, string ErrorMessage) InvokeFunction<TResult>(Func<TResult> function)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(_qualpaySettings.MerchantId) || !long.TryParse(_qualpaySettings.MerchantId, out var merchantId))
                    throw new QNetException("Plugin not configured");
            ...
            where TData : class
        {
            var (webhookData, errorMessage) = InvokeFunction(() =>
            {
                ...
                if (string.IsNullOrEmpty(message))
                    throw new QNetException("Webhook request is empty");
                ...
                if (!request.Headers.TryGetValue(QualpayDefaults.WebhookSignatureHeaderName, out var signatures))
                    throw new QNetException("Webhook request not signed by a signature header");
                ...
                if (!signatures.Any(signature => signature.Equals(encryptedString)))
                    throw new QNetException("Webhook request isn't valid");
        ...
        private void SendEmail(string email, bool subscribe)
        {
            ...
            var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId)
                ?? throw new QNetException("Email account could not be loaded");

            var subject = subscribe ? "New subscription" : "New unsubscription";
            var body = subscribe
                ? "QNet user just left the email to receive an information about special offers from Qualpay."
                : "QNet user has canceled subscription to receive Qualpay news.";
        ...
        private (IList<Domain.LineItem> Items, decimal TaxAmount) GetItems(Core.Domain.Customers.Customer customer, int storeId, decimal orderTotal)
        {
            ...
            var checkoutAttributesXml = _genericAttributeService
                .GetAttribute<string>(customer, QNetCustomerDefaults.CheckoutAttributes, storeId);
        ...
        private AddCustomerRequest CreateCustomerRequest(Core.Domain.Customers.Customer customer)
        {
            ...
            (
                ...
                customerFirstName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.FirstNameAttribute),
                customerLastName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.LastNameAttribute),
                customerFirmName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.CompanyAttribute),
                customerPhone: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.PhoneAttribute),
            ...
            switch (processPaymentRequest.RecurringCyclePeriod)
            {
                case RecurringProductCyclePeriod.Days:
                    ...
                    if (processPaymentRequest.RecurringCycleLength < 7)
                        throw new QNetException("Qualpay Payment Gateway error: Recurring Billing supports payments with the minimum frequency of once a week");
        ...
        private AddSubscriptionRequest CreateSubscriptionRequest(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (!_qualpaySettings.UseRecurringBilling)
                throw new QNetException("Recurring payments are not available");

            var customer = _customerService.GetCustomerById(processPaymentRequest.CustomerId)
                ?? throw new QNetException("Customer cannot be loaded");

            //Recurring Billing is available only for registered customers
            if (customer.IsGuest())
                throw new QNetException("Recurring payments are available only for registered customers");
            ...
            if (!primaryStoreCurrency.CurrencyCode.Equals("USD", StringComparison.InvariantCultureIgnoreCase))
                throw new QNetException("USD is not primary store currency");
            ...
            var vaultCustomer = GetCustomer(customer.Id) ?? CreateCustomer(customer)
                ?? throw new QNetException("Qualpay Payment Gateway error: Failed to create recurring payment.");
            ...
            if (selectedCard != null)
            {
                ...
                if (!selectedCard.Primary ?? true)
                {
                    ...
                    if (!updated)
                        throw new QNetException("Qualpay Payment Gateway error: Failed to pay by the selected card.");
                }
        ...
        private PGApiTransactionRequest CreateTransactionRequest(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            var primaryStoreCurrency = _currencyService.GetCurrencyById(_currencySettings.PrimaryStoreCurrencyId);
            if (!primaryStoreCurrency.CurrencyCode.Equals("USD", StringComparison.InvariantCultureIgnoreCase))
                throw new QNetException("USD is not a primary store currency");
            ...
            (
                ...
                developerId: $"{QualpayDefaults.DeveloperId}-{QNetVersion.CurrentVersion}",
        ...
        private string TokenizeCard(PGApiTokenizeRequest request)
        {
            return InvokeFunction(() =>
            {
                ...
                var response = apiClient.Tokenize(request)
                    ?? throw new QNetException("No response from service");

                //whether request is succeeded
                if (response.Rcode != "000")
                    throw new QNetException($"Error code - {response.Rcode}. {response.Rmsg}");
        ...
        private bool CreateCustomerCard(int customerId, AddBillingCardRequest request)
        {
            return InvokeFunction(() =>
            {
                ...
                //whether request is succeeded
                if (response.Code != 0)
                    throw new QNetException($"Error code - {response.Code}. {response.Message}");
