...
using QNet.Plugin.Payments.Square.Domain;
...
namespace QNet.Plugin.Payments.Square
{
    /// <summary>
    ...
    public class SquarePaymentMethod : BasePlugin, IPaymentMethod
    {
        ...
        private ProcessPaymentResult ProcessPayment(ProcessPaymentRequest paymentRequest, bool isRecurringPayment)
        {
            ...
            if (transaction == null)
                throw new QNetException(error);

            //get transaction tender
            var tender = transaction.Tenders?.FirstOrDefault();
            if (tender == null)
                throw new QNetException("There are no tenders (methods of payment) used to pay in the transaction");
        ...
        private ExtendedChargeRequest CreateChargeRequest(ProcessPaymentRequest paymentRequest, bool isRecurringPayment)
        {
            ...
            if (currency == null)
                throw new QNetException("Primary store currency cannot be loaded");

            //whether the currency is supported by the Square
            if (!CheckSupportCurrency(currency))
                throw new QNetException($"The {currency.CurrencyCode} currency is not supported by the Square");
            ...
            if (paymentRequest.CustomValues.TryGetValue(storedCardKey, out var storedCardId) && !storedCardId.ToString().Equals(Guid.Empty.ToString()))
            {
                ...
                if (squareCustomer == null)
                    throw new QNetException("Failed to retrieve customer");
            ...
            if (!paymentRequest.CustomValues.TryGetValue(cardNonceKey, out var cardNonce) || string.IsNullOrEmpty(cardNonce?.ToString()))
                throw new QNetException("Failed to get the card nonce");
            ...
            if (paymentRequest.CustomValues.TryGetValue(saveCardKey, out var saveCardValue) && saveCardValue is bool saveCard && saveCard && !customer.IsGuest())
            {
                ...
                try
                {
                    ...
                    if (squareCustomer == null)
                    {
                        ...
                        (
                            ...
                            GivenName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.FirstNameAttribute),
                            FamilyName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.LastNameAttribute),
                            PhoneNumber: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.PhoneAttribute),
                            CompanyName: _genericAttributeService.GetAttribute<string>(customer, QNetCustomerDefaults.CompanyAttribute),
                            ReferenceId: customer.CustomerGuid.ToString()
                        );
                        squareCustomer = _squarePaymentManager.CreateCustomer(customerRequest);
                        if (squareCustomer == null)
                            throw new QNetException("Failed to create customer. Error details in the log");
                    ...
                    if (card == null)
                        throw new QNetException("Failed to create card. Error details in the log");
                ...
                catch (Exception exception)
                {
                    ...
                    if (isRecurringPayment)
                        throw new QNetException("For recurring payments you need to save the card details");
                }
            }
        ...
        public RefundPaymentResult Refund(RefundPaymentRequest refundPaymentRequest)
        {
            ...
            if (transaction == null)
                throw new QNetException(transactionError);
            ...
            if (createdRefund == null)
                throw new QNetException(refundError);
        ...
        public override void Install()
        {
            ...
            //locales
            _localizationService.AddOrUpdatePluginLocaleResource("Enums.QNet.Plugin.Payments.Square.Domain.TransactionMode.Authorize", "Authorize only");
            _localizationService.AddOrUpdatePluginLocaleResource("Enums.QNet.Plugin.Payments.Square.Domain.TransactionMode.Charge", "Charge (authorize and capture)");
        ...
        public override void Uninstall()
        {
            ...
            //locales
            _localizationService.DeletePluginLocaleResource("Enums.QNet.Plugin.Payments.Square.Domain.TransactionMode.Authorize");
            _localizationService.DeletePluginLocaleResource("Enums.QNet.Plugin.Payments.Square.Domain.TransactionMode.Charge");
