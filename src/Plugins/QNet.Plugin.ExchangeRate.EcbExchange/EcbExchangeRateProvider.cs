...
namespace QNet.Plugin.ExchangeRate.EcbExchange
{
    public class EcbExchangeRateProvider : BasePlugin, IExchangeRateProvider
    {
        ...
        public IList<Core.Domain.Directory.ExchangeRate> GetCurrencyLiveRates(string exchangeRateCurrencyCode)
        {
            ...
            try
            {
                var httpClient = _httpClientFactory.CreateClient(QNetHttpDefaults.DefaultHttpClient);
            ...
            if (exchangeRateCurrency == null)
                throw new QNetException(_localizationService.GetResource("Plugins.ExchangeRate.EcbExchange.Error"));
